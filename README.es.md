...
Registra la herramienta `mini-advisor_echo`. Al llamarla responde `<greeting>: <text>`, donde `<greeting>` proviene de la configuración del plugin y `<text>` es el argumento.

## Install

```sh
pnpm pack
dsh plugin --profile <name> add ./dsh-mini-advisor-0.1.0.tgz
dsh --profile <name> --dump-config | grep 'dsh-mini-advisor'
```
